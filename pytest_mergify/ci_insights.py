...
🔍 Details: {self._flaky_detection_error_message}""",
    ...
    def _get_flaky_detection_report_message(self) -> str:
        ...
        for test_name, duration in self._new_test_durations_by_name.items():
            retry_count = self._new_test_retry_count_by_name.get(test_name, 0)
            if retry_count == 0:
                result += (
                    f"{os.linesep}    • '{test_name}' is too slow to be retried "
                    f"at least {_MIN_TEST_RETRY_COUNT} times within the budget"
                )
                continue

            test_duration_ms = int(duration.total_seconds() * 1000)
            test_retry_duration_ms = test_duration_ms * retry_count
