...
class FlakyDetectorMode(enum.Enum):
    NEW = "new"
    UNHEALTHY = "unhealthy"
...
class FlakyDetector:
    ...
    def _fetch_context(self) -> _FlakyDetectionContext:
        ...
        if self.mode == FlakyDetectorMode.NEW and len(result.existing_test_names) == 0:
            raise RuntimeError(
    ...
    def detect_from_report(self, report: _pytest.reports.TestReport) -> bool:
        ...
        if (
            self.mode == FlakyDetectorMode.NEW
            and test in self._context.existing_test_names
        ):
            return False
        elif (
            self.mode == FlakyDetectorMode.UNHEALTHY
            and test not in self._context.unhealthy_test_names
        ):
            return False
    ...
    def make_report(self) -> str:
        ...
        if not self._test_metrics:
            result += f"{os.linesep}- No {self.mode.value.lower()} tests detected, but we are watching 👀"
        ...
        result += (
            f"{os.linesep}- Active for {len(self._test_metrics)} {self.mode.value.lower()} "
