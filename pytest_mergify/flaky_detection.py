...
class FlakyDetector:
    ...
    _available_budget: datetime.timedelta = dataclasses.field(
    ...
    def set_test_to_process_from_session(self, session: _pytest.main.Session) -> None:
        ...
        if self.mode == "new":
            self._tests_to_process = [
                test for test in session_tests if test not in existing_tests_in_session
            ]
        ...
        self._available_budget = max(
            ratio * total_duration,
    ...
    def make_report(self) -> str:
        ...
        available_budget_duration_seconds = self._available_budget.total_seconds()
    ...
    def set_test_deadline(
        ...
        log_attrs = {
            "now": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "available_budget": str(self._available_budget),
    ...
    #     total_duration = self._context.existing_tests_mean_duration * self._foo
    ...
    def _get_remaining_budget_duration(self) -> datetime.timedelta:
        return max(
            self._available_budget - self._get_used_budget_duration(),
            datetime.timedelta(),
        )
