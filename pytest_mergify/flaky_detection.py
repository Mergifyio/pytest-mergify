...
class FlakyDetector:
    ...
    def filter_context_tests_for_session(self, session_tests: typing.Set[str]) -> None:
        self.log_messages.append(f"Session has {len(session_tests)} tests")
        ...
        self.log_messages.append(
            f"Filtered context: {original_existing_count} -> {len(self._context.existing_test_names)} existing tests, "
            f"{original_unhealthy_count} -> {len(self._context.unhealthy_test_names)} unhealthy tests"
        )
    ...
    def set_global_deadline(self) -> None:
        # Only set the deadline once, when the first test actually runs
        if self._global_deadline:
            self.log_messages.append(
                f"Global deadline already set to {self._global_deadline.isoformat()}, skipping"
            )
            return
    ...
    def _get_budget_duration(self) -> datetime.timedelta:
        # Use the filtered count of tests that are actually in the current session
        session_test_count = len(self._context.existing_test_names)
        total_duration = self._context.existing_tests_mean_duration * session_test_count
        self.log_messages.append(
            f"Calculated total duration for {session_test_count} session tests: {total_duration}"
        )
        ...
        budget_duration = max(
            total_duration + ratio * total_duration, self._context.min_budget_duration
        )
        self.log_messages.append(
